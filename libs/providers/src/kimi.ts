...
export class KimiProvider implements BaseProvider {
  ...
  constructor(apiKey: string, model?: string, baseURL?: string) {
    if (!apiKey) {
      throw new AIAdapterError('Kimi (Moonshot AI) API key is required', 'kimi');
    }

    this.client = new OpenAI({ apiKey, baseURL: baseURL || 'https://api.moonshot.ai/v1' });
  ...
  async chat(request: ChatRequest): Promise<ChatResponse> {
    try {
      ...
      throw new AIAdapterError(`Kimi chat failed: ${normalizedError.message}`, 'kimi', normalizedError);
    }
  }
}
