...
export class MistralProvider implements BaseProvider {
  ...
  constructor(apiKey: string, model?: string) {
    if (!apiKey) {
      throw new AIAdapterError('Mistral API key is required', 'mistral');
    }
  ...
  async chat(request: ChatRequest): Promise<ChatResponse> {
    try {
      ...
      throw new AIAdapterError(`Mistral chat failed: ${normalizedError.message}`, 'mistral', normalizedError);
    }
  }
}
